        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{ route('subcategories.create') }}" class="btn btn-primary">New Category</a>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
                    ...
                    <div class="card-header">
                        <div class="card-title">
                            <button class="btn btn-info" type="button"
                                onclick="window.location.href='{{ route('subcategories') }}'"">Reset</button>
                        </div>
                            ...
                            @if ($categories->isNotEmpty())
                                @foreach ($categories as $category)
                                    <tr>
                                        ...
                                        <td>{{ $category->slug }}</td>
                                        <td>
                                            @foreach ($category->parentCategories as $parent_cats)
                                                {{ $parent_cats->name }}
                                            @endforeach
                                        </td>
                                        <td>
                                            <form action="" method="">
                                                @if ($category->status == 1)
                                                    <a class="change-status" data-status="{{ $category->status }}"
                                                        data-id="{{ $category->id }}"><svg
                                                ...
                                                @else
                                                    <a class="change-status" data-status="{{ $category->status }}"
                                                        data-id="{{ $category->id }}"> <svg class="text-danger h-6 w-6"
                                                            xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                                                            aria-hidden="true">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <td>
                                            <a href="{{ route('subcategories.edit', ['id' => $category->id]) }}">
    ...
    <script>
        ...
        $('.delete-category').on('click', function(e) {
            ...
            if (confirm('Are you sure want to delete ?')) {
                ...
                var id = $(this).attr('data-id');
                var nurl = '{{ route('subcategories.delete', 'ID') }}';
